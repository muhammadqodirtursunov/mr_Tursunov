create database Sql_13_homework
use sql_13_homework
go
...
select * from cinema o
where id %2 = 1 and o.description != 'boring'
...
SELECT id, COALESCE(ssn, passportid, itin) AS first_non_null_id
FROM person;
...
SELECT 
  ...
  -- FirstName: boshidan birinchi bo‘shliqqacha
  LEFT(FullName, CHARINDEX(' ', FullName) - 1) AS FirstName,

  -- MiddleName: birinchi va ikkinchi bo‘shliq orasidagi so‘z
  SUBSTRING(
    FullName,
  ...
  -- LastName: ikkinchi bo‘shliqdan keyingi so‘z
  RIGHT(
    FullName,
...
select p.JobDescription, min(MissionCount) as minimum from Personal p
group by p.JobDescription


select p.JobDescription, max(MissionCount) as maximum from Personal p
group by p.JobDescription
...
-- Difficult Tasks
-- 1. Write an SQL query that separates the uppercase letters, lowercase letters, numbers, and other characters from the given string 
-- 'tf56sd#%OqH' into separate columns.

DECLARE @string VARCHAR(50) = 'tf56sd#%OqH';

select substring(@string, 1, [a-z]) from 
